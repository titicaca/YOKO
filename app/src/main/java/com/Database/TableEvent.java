    ...
    public List<EventRecord> queryEvent(long uid, long startTime, long endTime, int status, int modified) {
        ...
        try {
            cs = db.query(tableName, null,
                    ...
                    new String[]{String.valueOf(uid), String.valueOf(startTime), String.valueOf(endTime), String.valueOf(status), String.valueOf(modified)},
        ...
        try {
            cs = db.query(tableName, null,
                    DBConstants.COLUMN_EVENT_UID + " = ?" + " AND " +
                            DBConstants.COLUMN_EVENT_STATUS + " <= ?" + " AND " +
                            DBConstants.COLUMN_EVENT_MODIFIED + " <= ?",
                    new String[]{String.valueOf(uid), String.valueOf(status), String.valueOf(modified)},
                    null, null, DBConstants.COLUMN_EVENT_TIMEBEGIN);
            eventRecords = cursorToList(cs);
        } catch (Exception e) {
            e.printStackTrace();
